...
export function AdminEditWorker() {
  const { id } = useParams();
    ...
    const fetchWorker = async () => {
      try {
        ...
        setFormData({
          ...
          shift: worker.shift || 'Morning 8 Hours',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      // Remove password if empty to avoid accidental reset to empty string
      const payload: any = { ...formData };
      if (!payload.password) delete payload.password;
      
      await api.patch(`/admin/workers/${id}`, payload);
      navigate('/admin/workers');
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-800">Edit Worker</h2>
          <p className="text-slate-500">Update account details and work assignments for {formData.name}.</p>
        </div>
      </div>
